...
class TAG(object):
    ...
    def _render_buffer(self, buf, offset=None):
        raise NotImplementedError(self.__class__.__name__)
...
class _TAG_Numeric(TAG):
    ...
    def __init__(self, value=None, name=None, buf=None):
    ...
    # Parsers and Generators
    def _parse_buffer(self, buf, offset=None):
        self.value = self.fmt.unpack(buf.read(self.size))[0]
...
class TAG_Byte_Array(TAG):
    ...
    # Parsers and Generators
    def _parse_buffer(self, buf, offset=None):
        length = TAG_Int(buf=buf)
        self.value = buf.read(length.value)

    def _render_buffer(self, buf, offset=None):
        length = TAG_Int(len(self.value))
        length._render_buffer(buf, offset)
...
class TAG_String(TAG):
    ...
    # Parsers and Generators
    def _parse_buffer(self, buf, offset=None):
        length = TAG_Short(buf=buf)
        read = buf.read(length.value)
...
class TAG_List(TAG):
    ...
    def __init__(self, type=None, value=None, name=None, buf=None):
    ...
    # Parsers and Generators
    def _parse_buffer(self, buf, offset=None):
        self.tagID = TAG_Byte(buf=buf).value
        self.tags = []
        length = TAG_Int(buf=buf)
        for x in range(length.value):
            self.tags.append(TAGLIST[self.tagID](buf=buf))

    def _render_buffer(self, buf, offset=None):
        TAG_Byte(self.tagID)._render_buffer(buf, offset)
...
class TAG_Compound(TAG, DictMixin):
    ...
    def _parse_buffer(self, buf, offset=None):
        while True:
            type = TAG_Byte(buf=buf)
            if type.value == TAG_END:
                break
            else:
                name = TAG_String(buf=buf).value
                try:
                    tag = TAGLIST[type.value](buf=buf)
                    tag.name = name
    ...
    def _render_buffer(self, buf, offset=None):
        for tag in self.tags:
            TAG_Byte(tag.id)._render_buffer(buf, offset)
            TAG_String(tag.name)._render_buffer(buf, offset)
            tag._render_buffer(buf, offset)
...
class NBTFile(TAG_Compound):
    ...
    def parse_file(self, filename=None, buf=None, fileobj=None):
        if filename:
        ...
        if self.file:
            ...
            ValueError("need a file!")

    def write_file(self, filename=None, buf=None, fileobj=None):
        if buf:
            self.file = buf
        elif filename:
