...
def check_bounds(f):
    ...
    def deco(chunk, coords, *args, **kwargs):
        ...
        if not (0 <= x < 16 and 0 <= z < 16 and 0 <= y < CHUNK_HEIGHT):
            warn(
                "Coordinates %s are OOB in %s() of %s, ignoring call"
...
class Chunk(object):
    ...
    def __init__(self, x, z):
        ...
        :ivar array.array heightmap: Tracks the tallest block
        in each xz-column.
    ...
    def save_to_packet(self):
        ...
        packet = make_packet(
            "chunk", x=self.x, z=self.z, continuous=True,
    ...
    def set_block(self, coords, block):
        ...
        if self.get_block(coords) != block:
            ...
            if block in glowing_blocks:
                composite_glow(
                    self.blocklight, glowing_blocks[block], x, y, z
                )
